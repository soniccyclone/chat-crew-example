...
```
                    ...
                    POST /chat│        │ text/event-stream
                     (message)│        │ (agent activity events)
                              ▼        │
        ┌─────────────────────────────────────────────────┐
        │                 Litestar (ASGI)                  │
        │                                                   │
        │  OIDC login ── SessionAuth ── request.user        │
        │                                                   │
        │  ChatController                                   │
        │    • POST /chat  → enqueue turn, run agent in     │
        │                    worker thread                  │
        │    • GET  /chat/stream (SSE) → drain asyncio.Queue │
        │                                                   │
        │  ┌───────────────┐        ┌────────────────────┐  │
        │  │ CrewAI Flow   │──emits→│ Event bus listener │  │
        │  │ (conversational)│ events│ → per-session queue│  │
        │  │  + Tools       │        └────────────────────┘  │
        │  └───────┬────────┘                                │
        └──────────┼──────────────────┬─────────────────────┘
                   │                  │
          OpenAI /v1 (chat)      SQLAlchemy (async)
                   │                  │
          ┌────────▼───────┐   ┌──────▼───────┐   ┌─────────────┐
          │  LLM backend   │   │   Postgres   │   │  Keycloak   │
          │ (local/cloud)  │   │  (collection,│   │  (OIDC IdP) │
          └────────────────┘   │   messages)  │   └─────────────┘
                               └──────────────┘

   All of the above (except the LLM backend) runs as pods in a local
   kind cluster on podman; manifests generated by cdk8s (Python).
```

The LLM backend is drawn outside the cluster deliberately — see §6 and §8. Everything else is containerized and deployed via cdk8s-generated manifests into a local kind-on-podman cluster.
